...
def print_trainable_params(model):
    """
    Prints number in million total and trainable
    """
...
class TextVideoDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        training_shapes=[(128, 1, 128, 512, 512)],
    ):
    ...
    def load_frames_using_imageio(
        ...
        frames = []
        self.raw_frames = []
        self.raw_control_frames = []
        control_frames = []
        for frame_id in range(num_frames):
            video_frame_idx = start_frame_id + frame_id * interval
            frame = reader.get_data(video_frame_idx)
            ...
            video_dir = file_path.split('.mp4')[0]
            control_file_path = os.path.join(video_dir, 'pose', f'{video_frame_idx:06d}.png')
            control_frame = np.array(Image.open(control_file_path))
            self.raw_control_frames.append(control_frame)
            control_frame = torch.tensor(control_frame, dtype=torch.float32)
            control_frame = rearrange(control_frame, "H W C -> 1 C H W")
            control_frame = frame_process(control_frame)
        ...
        # --
        control_frames = torch.concat(control_frames, dim=0)
        control_frames = rearrange(control_frames, "T C H W -> C T H W")
        return frames, control_frames

    def load_video(self, file_path, training_shape_id, start_frame_id=None):
        data = {}
        max_num_frames, interval, num_frames, height, width \
            = self.training_shapes[training_shape_id]
        frame_process = self.frame_process[training_shape_id]
        if start_frame_id is None:
            start_frame_id = torch.randint(
                0, max_num_frames - (num_frames - 1) * interval, (1,)
            )[0]
        frames, control_frames = self.load_frames_using_imageio(
            file_path,
            max_num_frames,
            start_frame_id,
            interval,
        ...
        if frames is None:
            ...
            data[f"control_frames_{training_shape_id}"] = control_frames
            
        return data

    def __getitem__(self, index, start_frame_id=None):
        video_data = {}
        for training_shape_id in range(len(self.training_shapes)):
            while True:
                ...
                try:
                    data = self.load_video(video_file, training_shape_id, start_frame_id=start_frame_id)
                except:
                    data = None
                if data is not None:
                    break
...
class LightningModel(pl.LightningModule):
    def __init__(
        ...
        learning_rate=1e-5,
        svd_ckpt_path=None,
        add_positional_conv=128,
        contrast_enhance_scale=1.01,
    ):
        super().__init__()
        model_manager = ModelManager(torch_dtype=torch.float16, device=self.device)
        model_manager.load_stable_video_diffusion(
        ...
        for idx, block in enumerate(self.unet.blocks):
            if isinstance(block, TemporalAttentionBlock):# and idx % 2 == 0:
    ...
    def calculate_loss(self, frames, control_frames):
        with torch.no_grad():
            # Call video encoder
            latents = self.encode_video_with_vae(frames)

            control_latents = self.encode_video_with_vae(control_frames) / self.vae_encoder.scaling_factor
            # use first image as reference
            control_latents[:,:1] = latents[:,:1]
            ...
            add_time_id = torch.tensor(
                [[fps - 1, motion_bucket_id, noise_aug_strength]], device=self.device
            )

        # Calculate loss
        latents_input = torch.cat([noisy_latents, control_latents], dim=1)
        ...
        latents_output = self.noise_scheduler.step(
            model_pred.float(), timestep, noisy_latents.float(), to_final=True
        )
        loss = torch.nn.functional.mse_loss(
            latents_output,
            latents.float() * self.contrast_enhance_scale,
            reduction="mean",
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss, reweighted_loss = self.calculate_loss(frames, control_frames)
...
if __name__ == "__main__":
    ...
    dataset = TextVideoDataset(
        ...
        training_shapes=[
            (args.num_frames, 1, args.num_frames, args.height, args.width)
        ],
